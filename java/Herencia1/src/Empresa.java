public class Empresa {
	...
	public String Agregar(Empleado d){
		...
		if(i==-1){
			e[Contador++]=d;
			return "Se ha agregado el empleado";
		}
		else
	...
	public String Retirar(String k){
		...
		if(i!=-1){
			e[i]=null;
			Contador--;
			return "Se ha retirado el empleado";
		}
		else
	...
	public String Listar(int d,int k,int h){
		
		Ordenar();
		String Metodo="";
		for(int i=0;i<Contador;i++)
			Metodo=Metodo.concat(e[i].CalcularSueldo()+"\n");
		return Metodo+"La cantidad de gerente es de"+d+".La cantidad de supervisores es de"+k+".La cantidad de obreros es de"+h;
	}
